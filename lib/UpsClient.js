...
class UpsDevice {
  ...
  /** Get the value of a NUT variable.
    * @params {string} kay - The variable key.
  ...
  */
  constructor (params) {
    ...
    if (this._params.name == null) {
      ...
      if (this._params.name === 'localhost') {
        this._params.name = os.hostname().split('.')[0]
      }
    }
  ...
  async connect () {
    ...
    this._client = net.connect(this._params.port, this._params.hostname)
    this._client
      .on('connect', () => {
  ...
  async _send (command, expectedResponse) {
    await this.connect()
  ...
  async _get (type, key) {
    ...
    if (a != null && a[1] != null) {
      return homebridgeLib.OptionParser.patterns.int.test(a[1])
        ? parseInt(a[1])
        : a[1].trim()
    }
  }
  ...
  async _getMap (key) {
    ...
    for (const line of lines) {
      ...
      if (a != null && a[1] != null && a[2] != null) {
        map[a[1]] = homebridgeLib.OptionParser.patterns.int.test(a[2])
          ? parseInt(a[2])
          : a[2].trim()
      }
    }

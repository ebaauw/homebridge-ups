  ...
  async init () {
    try {
      ...
      for (const deviceId in devices) {
        ...
        const id = constants['device.serial'].trim().toUpperCase()
        if (this.upsAccessories[id] == null) {
          this.upsAccessories[id] = new UpsAccessory(this.platform, {
            name: description == null || description === 'Unavailable'
              ? this.client.name
              : description,
            id,
            manufacturer: constants['device.mfr'].trim(),
            model: constants['device.model'].trim(),
            firmware: constants['ups.firmware'].trim(),
            device,
            constants
          })
